...
public enum ImageTiling {
    ...
    public static func dedupedObservations(_ observations: [OCRObservation]) -> [OCRObservation] {
        ...
        for observation in observations {
            ...
            let winnerIsNew = observation.text.count > kept[index].text.count
            let winner = winnerIsNew ? observation : kept[index]
            let loser = winnerIsNew ? kept[index] : observation
            kept[index] = OCRObservation(
                text: winner.text,
                box: kept[index].box.union(observation.box),
                wordBoxes: wordBoxes(ofWinner: winner, alsoCovering: loser.box)
            )
        }
        return kept
    }

    /// Word boxes for a merged observation, or none when they cannot be trusted to cover it.
    ///
    /// Word boxes can only come from the read whose text won, since they are offsets into ITS text.
    /// That leaves them silent about ink only the losing read saw, so the question this answers is
    /// whether the loser's box already sits under the winner's words. Tiles are pixel-exact crops
    /// offset by whole pixels, so the usual case is one line read twice a pixel or two apart
    /// vertically at the same place along the line, and the winner's words cover it once they are
    /// grown to the full merged height.
    ///
    /// The case they cannot cover is the loser reaching somewhere the winner has no word — past the
    /// last one, or into the gap between two columns of a table, where a narrow cell can sit inside
    /// the wide line around it. Vertical drift of a pixel or two looks identical there, so no
    /// measure of the merged box separates the two. Only the loser's own box does, which is why it
    /// is passed in: the test is that the loser starts and ends where the winner's words do.
    /// Two reads of one line put its first and last word within this distance of each other. The
    /// measurement on a 1200x2600 capture was five orders of magnitude finer — about a
    /// hundred-thousandth of a pixel, because tiles are pixel-exact crops offset by whole pixels —
    /// and the thing being excluded is text found a whole word away, which is tens of pixels. A
    /// pixel sits between the two with room on both sides.
    static let sameLineTolerance: CGFloat = 1

    private static func wordBoxes(
        ofWinner winner: OCRObservation, alsoCovering loser: CGRect
    ) -> [OCRObservation.WordBox] {
        let spanned = winner.wordBoxes.map(\.box)
        guard let across = spanned.dropFirst().reduce(spanned.first, { $0?.union($1) }) else { return [] }
        guard abs(loser.minX - across.minX) <= sameLineTolerance,
              abs(loser.maxX - across.maxX) <= sameLineTolerance else { return [] }

        // Both ends line up, so the loser is the same run of words and the winner's boxes are over
        // its ink once they reach the merged height.
        let merged = winner.box.union(loser)
        return winner.wordBoxes.map {
            .init(range: $0.range,
                  box: CGRect(x: $0.box.minX, y: merged.minY, width: $0.box.width, height: merged.height))
        }
    }
