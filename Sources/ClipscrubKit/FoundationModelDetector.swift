...
// older SDKs, so the library keeps its macOS 14 floor. Generable conformances are written
// by hand because the @Generable / @Guide macro plugin ships with full Xcode but not the
// Command Line Tools, whose SDK still passes canImport — macros break plain `swift build`.
...
struct PHIFinding: Generable {
    ...
    static var generationSchema: GenerationSchema {
        GenerationSchema(type: Self.self, properties: [
            GenerationSchema.Property(
                name: "type",
                description: "An uppercase PHI/PII category label (e.g. NAME, SSN, ADDRESS, DOB); use OTHER if unsure",
                type: String.self
            ),
            GenerationSchema.Property(
                name: "value",
...
struct LLMPrompt: Codable {
    ...
    static let fallback = LLMPrompt(
        instructions: "You detect PHI and PII in text for redaction. Copy identifier values verbatim from the input.",
