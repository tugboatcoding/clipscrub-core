...
public enum DetectionSource: String, Codable, Sendable, CaseIterable {
    ...
    /// Matched a pattern the user wrote. Counts as deterministic corroboration when the optional
    /// model tier is gated — the user declared this shape an identifier, which is a stronger claim
    /// than the model guessing at one.
    case userRule
}
...
public struct OCRObservation: Sendable {
    /// Where one word inside an observation sits, so a span covering part of a line can be drawn
    /// over that part alone. `range` is in Characters, counted from the start of `text`.
    ...
    public let box: CGRect
    /// Vision reads a whole line as one observation, so `box` covers every word on that line. A
    /// redaction driven by `box` alone therefore takes out the line's other content too — an MRN
    /// and the diagnosis code printed beside it go together, and the reader loses the reason the
    /// page was being shared. These per-word boxes are what let the redactor cover just the MRN.
    ///
    /// Either every word of `text` is here or none of them are. A partial list would let a caller
    /// cover the words it knows about and leave the rest of an identifier readable, so the producer
    /// gives up the whole line rather than hand back a subset. Empty means callers fall back to
    /// `box`, which covers too much rather than too little — the opposite mistake leaves an
    /// identifier on screen.
    public let wordBoxes: [WordBox]

    public init(text: String, box: CGRect, wordBoxes: [WordBox] = []) {
        self.text = text
        self.box = box
